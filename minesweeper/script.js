...
// var BOMBS = 4;
// var bombList = [];
// const bombList = new bombList();
var revealedSquares = 0;
var cells = [];
var isGameOver = false;

function createField(ROWS, COLS, BOMBS) {
    cells = [];
    revealedSquares = 0;
    ...
    for(let i=0; i<ROWS; i++) {
        ...
        for(let j=0; j<COLS; j++) {
            ...
            var cell = new Cell(i, j, ROWS, COLS, BOMBS);
            cellRow.push(cell);
            
            if(bombList.containsBomb(i, j)) {
                cell.isBomb = true;
            }
            ...
            if(!cells[place[0]][place[1]].isFlagged) {
                if(cells[place[0]][place[1]].isBomb) {
                    if(revealedSquares != 0) {
                        ...
                        bombList.moveBomb(place[0], place[1], cells.BOMBS);
                        cells[place[0]][place[1]].reveal();
                    }
...
function start() {
    ...
    bombList.cols = COLS
    revealedSquares = 0;
    bombList.addBombs(BOMBS);
    createField(ROWS, COLS, BOMBS);
    isGameOver = false
}
...
var fieldEditors = document.getElementsByClassName("fieldEditors");
